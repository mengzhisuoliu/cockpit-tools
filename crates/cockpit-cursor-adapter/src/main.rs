use cockpit_core::models::cursor::{CursorAccount, CursorImportPayload};
use cockpit_core::models::{DefaultInstanceSettings, InstanceProfileView, InstanceStore};
use cockpit_core::modules::{cursor_account, cursor_instance, cursor_oauth, process};
...
fn to_value<T: Serialize>(value: T) -> Result<Value, String> {
    serde_json::to_value(value)
        .map_err(|error| format!("序列化 Cursor adapter 响应失败: {}", error))
}

fn parse_payload<T: for<'de> Deserialize<'de>>(payload: Value) -> Result<T, String> {
    serde_json::from_value(payload)
        .map_err(|error| format!("解析 Cursor adapter 请求失败: {}", error))
}
...
fn path_missing_error(error: &str) -> bool {
    error.starts_with("APP_PATH_NOT_FOUND:") || error.contains("启动 Cursor 失败")
}
...
fn default_instance_view(
    ...
    let running = settings
        .last_pid
        .and_then(|pid| cursor_instance::resolve_cursor_pid(Some(pid), None))
        .is_some();
...
async fn refresh_cursor_account_after_login(account: CursorAccount) -> CursorAccount {
...
fn add_with_token(payload: Value) -> Result<Value, String> {
    ...
    let account_payload = CursorImportPayload {
    ...
    to_value(cursor_account::upsert_account(account_payload)?)
}

fn complete_oauth(runtime: &Runtime, payload: Value) -> Result<Value, String> {
    let payload: LoginIdPayload = parse_payload(payload)?;
    let account_payload = runtime.block_on(cursor_oauth::complete_login(&payload.login_id))?;
    let account = cursor_account::upsert_account(account_payload)?;
    to_value(runtime.block_on(refresh_cursor_account_after_login(account)))
}
...
fn inject_bound_account_for_instance_start(
    ...
    let account = cursor_account::load_account(bind_id)
        .ok_or_else(|| format!("绑定账号不存在: {}", bind_id))?;
    cursor_instance::close_cursor(&[user_data_dir.to_string()], 20)?;
    cursor_instance::inject_account_to_profile(Path::new(user_data_dir), bind_id)?;
    eprintln!(
        "[CursorAdapter] injected bound account: account_id={}, email={}, user_data_dir={}",
...
fn list_instances() -> Result<Vec<InstanceProfileView>, String> {
    let store = cursor_instance::load_instance_store()?;
    let default_dir = cursor_instance::get_default_cursor_user_data_dir()?;
    let default_dir_str = default_dir.to_string_lossy().to_string();
    let default_settings = store.default_settings.clone();
    let process_entries = cursor_instance::collect_cursor_process_entries();

    let mut result: Vec<InstanceProfileView> = store
        .instances
        .into_iter()
        .map(|instance| {
            let resolved_pid = cursor_instance::resolve_cursor_pid_from_entries(
                instance.last_pid,
    ...
    let default_pid = cursor_instance::resolve_cursor_pid_from_entries(
...
fn create_instance(payload: Value) -> Result<Value, String> {
    ...
    let instance = cursor_instance::create_instance(cursor_instance::CreateInstanceParams {
        working_dir: None,
        name: payload.name,
...
fn update_instance(payload: Value) -> Result<Value, String> {
    ...
    if payload.instance_id == DEFAULT_INSTANCE_ID {
        let default_dir = cursor_instance::get_default_cursor_user_data_dir()?;
        let updated = cursor_instance::update_default_settings(
    ...
    let instance = cursor_instance::update_instance(cursor_instance::UpdateInstanceParams {
        working_dir: None,
    ...
    let running = instance
        .last_pid
        .and_then(|pid| {
            cursor_instance::resolve_cursor_pid(Some(pid), Some(&instance.user_data_dir))
        })
        .is_some();
    let initialized = is_profile_initialized(&instance.user_data_dir);
    to_value(InstanceProfileView::from_profile(
...
fn delete_instance(payload: Value) -> Result<Value, String> {
    ...
    cursor_instance::delete_instance(&payload.instance_id)?;
    Ok(Value::Null)
}
...
fn start_instance(payload: Value) -> Result<Value, String> {
    ...
    if payload.instance_id == DEFAULT_INSTANCE_ID {
        ...
        if let Some(pid) = cursor_instance::resolve_cursor_pid(default_settings.last_pid, None) {
            process::close_pid(pid, 20)?;
            let _ = cursor_instance::update_default_pid(None)?;
        }
        cursor_instance::close_cursor(&[default_dir_str.clone()], 20)?;
        ...
        let pid =
            cursor_instance::start_cursor_default_with_args_with_new_window(&extra_args, true)?;
        ...
        return to_value(InstanceProfileView {
            ...
            last_launched_at: None,
            last_pid: Some(pid),
            running: cursor_instance::resolve_cursor_pid(Some(pid), None).is_some(),
            initialized: is_profile_initialized(&default_dir.to_string_lossy()),
            is_default: true,
            follow_local_account: false,
        });
    ...
    if let Some(pid) =
        cursor_instance::resolve_cursor_pid(instance.last_pid, Some(&instance.user_data_dir))
    {
        process::close_pid(pid, 20)?;
        let _ = cursor_instance::update_instance_pid(&instance.id, None)?;
    }
    cursor_instance::close_cursor(&[instance.user_data_dir.clone()], 20)?;
    inject_bound_account_for_instance_start(
        &instance.user_data_dir,
        instance.bind_account_id.as_deref(),
    )?;
    let extra_args = process::parse_extra_args(&instance.extra_args);
    let pid = cursor_instance::start_cursor_with_args_with_new_window(
        &instance.user_data_dir,
        &extra_args,
        true,
    )?;
    let updated = cursor_instance::update_instance_after_start(&instance.id, pid)?;
    let running =
        cursor_instance::resolve_cursor_pid(Some(pid), Some(&updated.user_data_dir)).is_some();
...
fn stop_instance(payload: Value) -> Result<Value, String> {
    ...
    if payload.instance_id == DEFAULT_INSTANCE_ID {
        ...
        let updated = cursor_instance::update_default_pid(None)?;
        return to_value(InstanceProfileView {
            id: DEFAULT_INSTANCE_ID.to_string(),
            name: String::new(),
            user_data_dir: default_dir_str,
    ...
    let updated = cursor_instance::update_instance_pid(&instance.id, None)?;
    let initialized = is_profile_initialized(&updated.user_data_dir);
    to_value(InstanceProfileView::from_profile(
        updated,
        false,
...
fn open_instance_window(payload: Value) -> Result<Value, String> {
    let payload: InstanceIdPayload = parse_payload(payload)?;
    if payload.instance_id == DEFAULT_INSTANCE_ID {
        let default_settings: DefaultInstanceSettings = cursor_instance::load_default_settings()?;
        cursor_instance::focus_cursor_instance(default_settings.last_pid, None)
    ...
    let instance = store
        ...
        .find(|item| item.id == payload.instance_id)
        .ok_or("实例不存在")?;
    cursor_instance::focus_cursor_instance(instance.last_pid, Some(&instance.user_data_dir))
        .map_err(|err| {
            format!(
...
fn close_all_instances() -> Result<Value, String> {
    ...
    cursor_instance::close_cursor(&target_dirs, 20)?;
    let _ = cursor_instance::clear_all_pids();
    Ok(Value::Null)
}

fn switch_inject(payload: Value) -> Result<Value, String> {
    let payload: AccountIdPayload = parse_payload(payload)?;
    let account = cursor_account::load_account(&payload.account_id)
        .ok_or_else(|| format!("Cursor account not found: {}", payload.account_id))?;
    cursor_account::inject_to_cursor(&payload.account_id)?;
    let _ = cursor_instance::update_default_settings(
    ...
    match start_instance(json!({ "instanceId": DEFAULT_INSTANCE_ID })) {
        ...
        Err(error) if path_missing_error(&error) => to_value(SwitchResult {
            message: format!("切换完成，但 Cursor 启动失败: {}", error),
...
fn inject_default_profile(payload: Value) -> Result<Value, String> {
    let payload: AccountIdPayload = parse_payload(payload)?;
    let default_dir = cursor_instance::get_default_cursor_user_data_dir()?;
    cursor_instance::inject_account_to_profile(default_dir.as_path(), &payload.account_id)?;
    Ok(Value::Null)
}
...
fn handle_rpc(runtime: &Runtime, request: RpcRequest) -> Result<Value, String> {
    match request.method.as_str() {
        ...
        "accounts.list" => to_value(cursor_account::list_accounts_checked()?),
        "accounts.current" => to_value(cursor_account::resolve_current_account_id(
            &cursor_account::list_accounts_checked()?,
        )),
        "accounts.delete" => {
            let payload: AccountIdPayload = parse_payload(request.payload)?;
            cursor_account::remove_account(&payload.account_id)?;
            Ok(Value::Null)
        }
        "accounts.deleteMany" => {
            let payload: AccountIdsPayload = parse_payload(request.payload)?;
            cursor_account::remove_accounts(&payload.account_ids)?;
            Ok(Value::Null)
        }
        "accounts.importJson" => {
            let payload: JsonImportPayload = parse_payload(request.payload)?;
            to_value(cursor_account::import_from_json(&payload.json_content)?)
        }
        "accounts.importLocal" => import_local(),
        "accounts.addToken" => add_with_token(request.payload),
        "accounts.export" => {
            let payload: AccountIdsPayload = parse_payload(request.payload)?;
            to_value(cursor_account::export_accounts(&payload.account_ids)?)
        }
        "accounts.indexPath" => to_value(cursor_account::accounts_index_path_string()?),
        "accounts.refresh" => {
            let payload: AccountIdPayload = parse_payload(request.payload)?;
            let account =
                runtime.block_on(cursor_account::refresh_account_async(&payload.account_id))?;
            let _ = cursor_account::run_quota_alert_if_needed();
            to_value(account)
        }
        "accounts.refreshAll" => {
            let results = runtime.block_on(cursor_account::refresh_all_tokens())?;
        ...
        "accounts.updateTags" => {
            ...
            to_value(cursor_account::update_account_tags(
                &payload.account_id,
                payload.tags,
            )?)
        }
        "quota.alertPayload" => to_value(cursor_account::run_quota_alert_if_needed()?),
        "oauth.start" => to_value(cursor_oauth::start_login()?),
        "oauth.complete" => complete_oauth(runtime, request.payload),
        "oauth.cancel" => {
            let payload: LoginCancelPayload = parse_payload(request.payload)?;
            cursor_oauth::cancel_login(payload.login_id.as_deref())?;
            Ok(Value::Null)
        }
        "switch.inject" => switch_inject(request.payload),
        "switch.injectDefaultProfile" => inject_default_profile(request.payload),
        "instances.store.get" => to_value(cursor_instance::load_instance_store()?),
        "instances.store.replace" => {
            ...
            cursor_instance::save_instance_store(&store)?;
            Ok(Value::Null)
        }
        "instance.getDefaults" => to_value(cursor_instance::get_instance_defaults()?),
        ...
        "runtime.detectLaunchPath" => {
            ...
            to_value(cursor_instance::detect_and_save_cursor_launch_path(
        ...
        other => Err(format!("未知 Cursor adapter 方法: {}", other)),
    }
}
...
fn write_json_response(request: tiny_http::Request, status: u16, response: RpcResponse) {
    let body = serde_json::to_string(&response).unwrap_or_else(|error| {
        serde_json::json!({
            "ok": false,
            "error": { "message": format!("序列化 Cursor adapter HTTP 响应失败: {}", error) }
        })
        .to_string()
    });
...
fn handle_http_request(
    ...
    if request.method() != &Method::Post || request.url() != "/rpc" {
        write_json_response(
            request,
            404,
            error_response("Cursor adapter 路由不存在".to_string()),
        );
        return;
    }
    if !is_authorized(&request, &token) {
        write_json_response(
            request,
            401,
            error_response("Cursor adapter token 无效".to_string()),
        );
        return;
    }
    ...
    if let Err(error) = request.as_reader().read_to_string(&mut body) {
        write_json_response(
            request,
            400,
            error_response(format!("读取 Cursor adapter 请求失败: {}", error)),
        );
        return;
    }
    ...
    let rpc_request = match serde_json::from_str::<RpcRequest>(&body) {
        ...
        Err(error) => {
            write_json_response(
                request,
                400,
                error_response(format!("解析 Cursor adapter 请求 JSON 失败: {}", error)),
            );
            return;
        }
    };
...
fn main() {
    ...
    let server = Server::http("127.0.0.1:0").expect("bind cursor adapter server");
    let address = server.server_addr().to_string();
    let port = address
        .rsplit_once(':')
        .and_then(|(_, port)| port.parse::<u16>().ok())
        .expect("parse cursor adapter port");
    ...
    while !shutdown.load(Ordering::SeqCst) {
        match server.recv_timeout(Duration::from_millis(200)) {
            ...
            Ok(None) => {}
            Err(error) => {
                eprintln!("Cursor adapter request receive failed: {}", error);
                break;
            }
        }
    }
