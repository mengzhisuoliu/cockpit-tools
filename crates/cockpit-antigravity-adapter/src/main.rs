...
fn handle_http_request(
    ...
    match handle_rpc(runtime.as_ref(), rpc_request) {
        Ok(data) => {
            if is_shutdown {
...
fn main() {
    ...
    let runtime = Arc::new(Runtime::new().expect("create Antigravity adapter tokio runtime"));
    ...
    while !shutdown.load(Ordering::SeqCst) {
        match server.recv_timeout(Duration::from_millis(200)) {
            ...
            Ok(None) => {}
            Err(error) => {
                eprintln!("Antigravity adapter request receive failed: {}", error);
                break;
            }
        }
    }
