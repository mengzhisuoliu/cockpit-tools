...
use cockpit_core::modules::{
    config, github_copilot_account, github_copilot_instance, github_copilot_oauth, opencode_auth,
...
fn to_value<T: Serialize>(value: T) -> Result<Value, String> {
    serde_json::to_value(value)
        .map_err(|error| format!("序列化 GitHub Copilot adapter 响应失败: {}", error))
}

fn parse_payload<T: for<'de> Deserialize<'de>>(payload: Value) -> Result<T, String> {
    serde_json::from_value(payload)
        .map_err(|error| format!("解析 GitHub Copilot adapter 请求失败: {}", error))
}
...
fn path_missing_error(error: &str) -> bool {
    error.starts_with("APP_PATH_NOT_FOUND:") || error.contains("启动 VS Code 失败")
}
...
fn default_instance_view(
    ...
    let running = settings
        .last_pid
        .and_then(|pid| process::resolve_vscode_pid(Some(pid), None))
        .is_some();
...
async fn refresh_github_copilot_account_after_login(
    account: cockpit_core::models::github_copilot::GitHubCopilotAccount,
) -> cockpit_core::models::github_copilot::GitHubCopilotAccount {
...
fn import_local(runtime: &Runtime) -> Result<Value, String> {
    match runtime.block_on(github_copilot_account::import_from_local())? {
...
fn add_with_token(runtime: &Runtime, payload: Value) -> Result<Value, String> {
    ...
    let account_payload = runtime.block_on(
        github_copilot_oauth::build_payload_from_github_access_token(&payload.github_access_token),
    )?;
    to_value(github_copilot_account::upsert_account(account_payload)?)
}

fn complete_oauth(runtime: &Runtime, payload: Value) -> Result<Value, String> {
    let payload: LoginIdPayload = parse_payload(payload)?;
    let account_payload =
        runtime.block_on(github_copilot_oauth::complete_login(&payload.login_id))?;
    let account = github_copilot_account::upsert_account(account_payload)?;
    to_value(runtime.block_on(refresh_github_copilot_account_after_login(account)))
}
...
fn inject_bound_account_for_instance_start(
    ...
    let account = github_copilot_account::load_account(bind_id)
        .ok_or_else(|| format!("绑定账号不存在: {}", bind_id))?;
    process::close_vscode(&[user_data_dir.to_string()], 20)?;
    let github_id = account.github_id.to_string();
    vscode_inject::inject_copilot_token_for_user_data_dir(
        user_data_dir,
    ...
    .map_err(|error| {
        format!(
            "按绑定账号注入实例失败（{}）: {}",
            account.github_login, error
        )
    })
}

fn list_instances() -> Result<Vec<InstanceProfileView>, String> {
    let store = github_copilot_instance::load_instance_store()?;
    let default_dir = github_copilot_instance::get_default_vscode_user_data_dir()?;
    let default_dir_str = default_dir.to_string_lossy().to_string();
    let default_settings = store.default_settings.clone();
    let process_entries = process::collect_vscode_process_entries();

    let mut result: Vec<InstanceProfileView> = store
        .instances
        .into_iter()
        .map(|instance| {
            let resolved_pid = process::resolve_vscode_pid_from_entries(
                instance.last_pid,
    ...
    let default_pid =
        process::resolve_vscode_pid_from_entries(default_settings.last_pid, None, &process_entries);
...
fn create_instance(payload: Value) -> Result<Value, String> {
    ...
    let instance =
        github_copilot_instance::create_instance(github_copilot_instance::CreateInstanceParams {
            working_dir: None,
            name: payload.name,
...
fn update_instance(payload: Value) -> Result<Value, String> {
    ...
    if payload.instance_id == DEFAULT_INSTANCE_ID {
        let default_dir = github_copilot_instance::get_default_vscode_user_data_dir()?;
        let updated = github_copilot_instance::update_default_settings(
    ...
    let instance =
        github_copilot_instance::update_instance(github_copilot_instance::UpdateInstanceParams {
            working_dir: None,
    ...
    let running = instance
        .last_pid
        .and_then(|pid| process::resolve_vscode_pid(Some(pid), Some(&instance.user_data_dir)))
        .is_some();
    let initialized = is_profile_initialized(&instance.user_data_dir);
    to_value(InstanceProfileView::from_profile(
        instance,
...
fn delete_instance(payload: Value) -> Result<Value, String> {
    ...
    github_copilot_instance::delete_instance(&payload.instance_id)?;
    Ok(Value::Null)
}

fn start_instance(payload: Value) -> Result<Value, String> {
    let payload: InstanceIdPayload = parse_payload(payload)?;
    process::ensure_vscode_launch_path_configured()?;

    if payload.instance_id == DEFAULT_INSTANCE_ID {
        let default_dir = github_copilot_instance::get_default_vscode_user_data_dir()?;
        let default_dir_str = default_dir.to_string_lossy().to_string();
        let default_settings = github_copilot_instance::load_default_settings()?;
        if let Some(pid) = process::resolve_vscode_pid(default_settings.last_pid, None) {
            process::close_pid(pid, 20)?;
            let _ = github_copilot_instance::update_default_pid(None)?;
        }

        process::close_vscode(&[default_dir_str.clone()], 20)?;
        ...
        let pid = process::start_vscode_default_with_args_with_new_window(&extra_args, true)?;
        let _ = github_copilot_instance::update_default_pid(Some(pid))?;
    ...
    if let Some(pid) = process::resolve_vscode_pid(instance.last_pid, Some(&instance.user_data_dir))
    {
        process::close_pid(pid, 20)?;
        let _ = github_copilot_instance::update_instance_pid(&instance.id, None)?;
    }
    process::close_vscode(&[instance.user_data_dir.clone()], 20)?;
    inject_bound_account_for_instance_start(
    ...
    let pid = process::start_vscode_with_args_with_new_window(
        &instance.user_data_dir,
        &extra_args,
        true,
    )?;
    let updated = github_copilot_instance::update_instance_after_start(&instance.id, pid)?;
...
fn stop_instance(payload: Value) -> Result<Value, String> {
    ...
    if payload.instance_id == DEFAULT_INSTANCE_ID {
        ...
        let updated = github_copilot_instance::update_default_pid(None)?;
        return to_value(default_instance_view(&default_dir, updated));
    }
    ...
    let updated = github_copilot_instance::update_instance_pid(&instance.id, None)?;
    let initialized = is_profile_initialized(&updated.user_data_dir);
    to_value(InstanceProfileView::from_profile(
        updated,
        false,
...
fn open_instance_window(payload: Value) -> Result<Value, String> {
    ...
    if payload.instance_id == DEFAULT_INSTANCE_ID {
        let default_settings: DefaultInstanceSettings =
            github_copilot_instance::load_default_settings()?;
        process::focus_vscode_instance(default_settings.last_pid, None)
            .map_err(|error| format!("定位 VS Code 默认实例窗口失败: {}", error))?;
    ...
    let instance = store
        ...
        .find(|item| item.id == payload.instance_id)
        .ok_or("实例不存在")?;
    process::focus_vscode_instance(instance.last_pid, Some(&instance.user_data_dir)).map_err(
        |error| {
            format!(
...
fn close_all_instances() -> Result<Value, String> {
    ...
    process::close_vscode(&target_dirs, 20)?;
    let _ = github_copilot_instance::clear_all_pids();
    Ok(Value::Null)
}

fn switch_inject(payload: Value) -> Result<Value, String> {
    let payload: AccountIdPayload = parse_payload(payload)?;
    let account = github_copilot_account::load_account(&payload.account_id)
        .ok_or_else(|| format!("GitHub Copilot account not found: {}", payload.account_id))?;
    let _ = github_copilot_instance::update_default_settings(
    ...
    if user_config.ghcp_opencode_sync_on_switch
        && user_config.ghcp_opencode_auth_overwrite_on_switch
        && opencode_updated
    {
        if process::is_opencode_running() {
            let _ = process::close_opencode(20);
        }
    ...
    match start_instance(json!({ "instanceId": DEFAULT_INSTANCE_ID })) {
        ...
        Err(error) if path_missing_error(&error) => to_value(SwitchResult {
            message: format!("切换完成，但 VS Code 启动失败: {}", error),
...
fn handle_rpc(runtime: &Runtime, request: RpcRequest) -> Result<Value, String> {
    match request.method.as_str() {
        ...
        "accounts.list" => to_value(github_copilot_account::list_accounts_checked()?),
        "accounts.current" => to_value(github_copilot_account::resolve_current_account_id(
            &github_copilot_account::list_accounts_checked()?,
        )),
        "accounts.delete" => {
            let payload: AccountIdPayload = parse_payload(request.payload)?;
            github_copilot_account::remove_account(&payload.account_id)?;
            Ok(Value::Null)
        }
        "accounts.deleteMany" => {
            let payload: AccountIdsPayload = parse_payload(request.payload)?;
            github_copilot_account::remove_accounts(&payload.account_ids)?;
            Ok(Value::Null)
        }
        "accounts.importJson" => {
            let payload: JsonImportPayload = parse_payload(request.payload)?;
            to_value(github_copilot_account::import_from_json(
        ...
        "accounts.export" => {
            let payload: AccountIdsPayload = parse_payload(request.payload)?;
            to_value(github_copilot_account::export_accounts(
                &payload.account_ids,
            )?)
        }
        "accounts.indexPath" => to_value(github_copilot_account::accounts_index_path_string()?),
        "accounts.refresh" => {
            let payload: AccountIdPayload = parse_payload(request.payload)?;
            let account = runtime.block_on(github_copilot_account::refresh_account_token(
                &payload.account_id,
            ))?;
            let _ = github_copilot_account::run_quota_alert_if_needed();
            to_value(account)
        }
        "accounts.refreshAll" => {
            let results = runtime.block_on(github_copilot_account::refresh_all_tokens())?;
        ...
        "accounts.updateTags" => {
            ...
            to_value(github_copilot_account::update_account_tags(
                &payload.account_id,
                payload.tags,
            )?)
        }
        ...
        "oauth.start" => to_value(runtime.block_on(github_copilot_oauth::start_login())?),
        "oauth.complete" => complete_oauth(runtime, request.payload),
        "oauth.cancel" => {
            let payload: LoginCancelPayload = parse_payload(request.payload)?;
            github_copilot_oauth::cancel_login(payload.login_id.as_deref())?;
            Ok(Value::Null)
        }
        "oauth.restorePendingListener" => Ok(Value::Null),
        "switch.inject" => switch_inject(request.payload),
        "instances.store.get" => to_value(github_copilot_instance::load_instance_store()?),
        "instances.store.replace" => {
            ...
            github_copilot_instance::save_instance_store(&store)?;
            Ok(Value::Null)
        }
        "instance.getDefaults" => to_value(github_copilot_instance::get_instance_defaults()?),
        ...
        other => Err(format!("未知 GitHub Copilot adapter 方法: {}", other)),
    }
}
...
fn write_json_response(request: tiny_http::Request, status: u16, response: RpcResponse) {
    let body = serde_json::to_string(&response).unwrap_or_else(|error| {
        serde_json::json!({
            "ok": false,
            "error": { "message": format!("序列化 GitHub Copilot adapter HTTP 响应失败: {}", error) }
        })
        .to_string()
    });
...
fn handle_http_request(
    ...
    if request.method() != &Method::Post || request.url() != "/rpc" {
        write_json_response(
            request,
            404,
            error_response("GitHub Copilot adapter 路由不存在".to_string()),
        );
        return;
    }
    if !is_authorized(&request, &token) {
        write_json_response(
            request,
            401,
            error_response("GitHub Copilot adapter token 无效".to_string()),
        );
        return;
    }
    ...
    if let Err(error) = request.as_reader().read_to_string(&mut body) {
        write_json_response(
            request,
            400,
            error_response(format!("读取 GitHub Copilot adapter 请求失败: {}", error)),
        );
        return;
    }
        ...
        Err(error) => {
            write_json_response(
                ...
                error_response(format!(
                    "解析 GitHub Copilot adapter 请求 JSON 失败: {}",
                    error
                )),
            );
            return;
...
fn main() {
    ...
    let server = Server::http("127.0.0.1:0").expect("bind github copilot adapter server");
    let address = server.server_addr().to_string();
    let port = address
        .rsplit_once(':')
        .and_then(|(_, port)| port.parse::<u16>().ok())
        .expect("parse github copilot adapter port");
    ...
    while !shutdown.load(Ordering::SeqCst) {
        match server.recv_timeout(Duration::from_millis(200)) {
            ...
            Ok(None) => {}
            Err(error) => {
                eprintln!("GitHub Copilot adapter request receive failed: {}", error);
                break;
            }
        }
    }
